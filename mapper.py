...
class Auftrag(Base):
    ...
    Montage = relationship('Montage', back_populates="ListeAuftrag")
...
class Ersatzteil(Base):
    ...
    EtID = Column('EtID', String(5), primary_key=True)
...
class Montage(Base):
    ...
    EtID = Column('EtID', String(5), ForeignKey(Ersatzteil.EtID) , primary_key=True)
    AufNr = Column('AufNr', Integer, ForeignKey(Auftrag.AufNr) ,primary_key=True)
    Anzahl = Column('Anzahl', Integer)
    ListeAuftrag  = relationship("Auftrag", back_populates="Montage")
